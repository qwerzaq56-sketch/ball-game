...
`F1` 디버그 패널의 **Gameplay** 섹션 체크박스로 `player.allyAbsorptionEnabled`를 토글한다
(`js/ui.js#buildGameplaySection`). OFF 상태에서는 `Game.resolveConsumption()`이 플레이어가
흡수자인 경우에 한해 흡수 시작 자체를 건너뛴다 — 아군에 대한 다른 상호작용(밀어내기는
애초에 같은 색끼리는 적용 안 됨, 공격도 같은 색은 `isHostile()`이 걸러서 불가능)은 전혀
영향받지 않는다. AI는 이 토글과 무관하게 자신의 흡수 로직(§7)을 그대로 따른다 — 이건
플레이어 전용 컨트롤이다.

최초 구현은 캔버스 우클릭으로 토글했는데, 전투 중 실수로 눌리기 쉬워 의도치 않게 꺼지는
사고가 발생할 수 있었다 — 디버그 패널 뒤로 옮겨 "명시적으로 패널을 열고 클릭해야만 바뀌는"
동작으로 바꿨다. 체크박스는 매 프레임 `player.allyAbsorptionEnabled`와 동기화되므로,
`Game.reset()`으로 값이 기본값(ON)으로 되돌아가도 체크 표시가 항상 실제 상태를 따라간다.
...
Orb 크기 범위   = minOrbSize ~ minOrbSize + max(필드 Orb 자연 범위, 적 Size × orbSizeGrowthPerEnemySize)
Orb 성장치      = 필드 Orb와 동일한 공식으로 각 Orb 자신의 크기에서 계산 (5 + (size - minOrbSize) × 2)
퍼짐 반경       = 적 Size × orbSpreadMultiplier(2.0) + orbSpreadBase(20)
```

`js/game.js#onEntityDeath`, `js/spawning.js#spawnDeathOrbs`. 처치 즉시 받는 Growth를
절반으로 줄이고, 대신 시체 자리에 떨어지는 Orb로 보상 대부분을 옮겼다 — 플레이어는 그 보상을
직접 걸어가서 모아야 하므로, 처치 직후에도 위험한 전장에 잠시 더 머물러야 하는 순간이
생긴다("처치 = 즉시 안전하게 강해짐"이 아니라 "처치 = 위험을 감수하고 거둬야 할 보상").

**후속 조정 (같은 v0.6 내)**: 처음 구현은 Orb 크기와 성장치를 고정값으로 뒀는데, 두 가지
문제가 드러났다. (1) 모든 드랍 Orb가 똑같이 생겨서 필드에 자연스폰되는 Orb와 다르게
"반짝이는 작은 점"처럼 보였고(십자 마크까지 더해져 더 도드라졌다), (2) Orb *개수*가
`orbMaxCount`에서 캡되는 순간부터 총 보상이 완전히 멈췄다 — 개별 Orb 가치가 고정이었으므로
`count`가 상한에 닿으면 그게 곧 총 보상의 상한이었다.

수정: Orb 크기를 필드 Orb와 완전히 같은 방식(범위 내 무작위)으로 생성하되, 그 범위 자체가
죽은 적의 Size에 비례해 넓어지게 했고, 성장치도 필드 Orb와 동일한 밀도 공식으로 각 Orb
자신의 크기에서 계산하도록 바꿨다 — 더 이상 어떤 것도 고정값이 아니다. 그 결과 Orb 개수가
상한에 도달한 뒤에도 **개별 Orb의 가치**가 Size에 비례해 계속 커지므로, 총 보상은 구조적으로
정체점을 갖지 않는다(실측: Size 200→500 구간에서 개수는 1.7배만 늘었지만 총 Growth는
4.3배 늘었다 — BALANCE_NOTES 참고). 퍼짐 반경도 배율을 키워 큰 적을 잡을수록 보상이 눈에
띄게 넓은 영역에 흩뿌려지게 했다. 십자 마크 렌더링은 완전히 제거해 필드 Orb와 시각적으로
구분되지 않게 했다.

Fragment는 v0.2부터 이미 Orb로 완전히 통합되어 있었다(별도 타입/렌더링/충돌 경로가 존재한
적이 없음) — v0.6에서 새로 제거할 것은 없었고, 이번 처치-보상 개편이 "죽인 적이 남기는 것도
결국 그냥 Orb"라는 그 원칙을 한 번 더 철저하게 재확인한다.
