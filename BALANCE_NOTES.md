...
## [추가 수정] 아군 흡수 토글을 우클릭에서 디버그 패널로 이동

사용자 요청: 우클릭 제스처가 전투 중 실수로 눌리기 쉬워서, 의도치 않게 아군 흡수가 꺼지는
사고가 발생할 수 있었다. `js/main.js`의 `contextmenu` 리스너에서 토글 로직을 완전히
제거하고(컨텍스트 메뉴 자체는 여전히 막아서 우클릭이 브라우저 메뉴를 띄우지는 않는다),
`F1` 디버그 패널의 새 **Gameplay** 섹션에 체크박스로 옮겼다 — 패널을 열고 명시적으로
클릭해야 하므로 실수로 꺼질 가능성이 훨씬 낮다. 기본값은 그대로 ON(`player.js`의
`allyAbsorptionEnabled = true`).

체크박스는 `Game.reset()`으로 새 Player가 생성될 때마다(항상 ON으로 시작) `UI.update()`가
매 프레임 실제 상태와 동기화하므로, 리셋 후에도 체크 표시가 항상 실제 게임 상태와 일치한다.

실측(실제 UI 클릭 + 콘솔 조작):
- 체크박스 클릭 → HUD의 "ALLY ABSORB"가 즉시 ON↔OFF로 전환.
- 임의로 OFF 상태를 만든 뒤 `Game.reset()` 호출 → 체크박스가 자동으로 다시 체크된 상태로
  복귀(수동 갱신 없이, 다음 프레임의 `ui.update()`에서 동기화됨).
- 캔버스에 `contextmenu` 이벤트를 직접 디스패치해도 `player.allyAbsorptionEnabled` 값이
  전혀 변하지 않는 것을 확인 — 우클릭 경로가 완전히 제거됐다.
